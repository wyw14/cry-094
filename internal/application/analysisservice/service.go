...
func (s *Service) Analyze(ctx context.Context, teamID, actorID, libraryID string, artifactIDs []string, inventory host.Inventory) (*analysis.Result, error) {
	...
	defer cancel()
	owner, err := s.teams.Get(workCtx, teamID)
	if err != nil {
		return nil, err
	}
	...
	library, err := s.artifacts.GetLibrary(workCtx, libraryID)
	if err != nil {
		return nil, err
	}
	...
	for _, id := range artifactIDs {
		...
		artifact, err := s.artifacts.GetArtifact(workCtx, id)
		if err != nil {
			return nil, err
		}
		...
		content, err := s.store.Get(workCtx, artifact.StorageKey)
		if err != nil {
			return nil, err
		}
		...
		conclusion, err := parser.Parse(workCtx, *artifact, content)
		if err != nil {
			return nil, err
		}
	...
	if err := s.results.Save(workCtx, &result); err != nil {
		return nil, err
	}
	return &result, nil
}
		...
		for _, dependency := range conclusion.Dependencies {
			...
			case script.DependencyTool:
				...
				if !exists {
					...
					issues = append(issues, analysis.Issue{Kind: analysis.IssueVersionConflict, Severity: analysis.SeverityBlocking, Summary: "target tool version does not satisfy the script constraint", Evidence: []analysis.EvidenceStep{{ArtifactID: artifact.ID, Dependency: dependency.Name, Detail: fmt.Sprintf("host has %s but script requires %s", tool.Version.String(), constraint), Line: dependency.Location.Line}}, Resolution: "use a compatible host or revise the declared version constraint"})
				}
