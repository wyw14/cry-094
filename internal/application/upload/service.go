...
func (s *Service) Upload(ctx context.Context, request Request) (*script.Artifact, error) {
	...
	id := s.ids.NewID()
	storageKey := fmt.Sprintf("objects/%s", digest)
	...
	if _, err := s.artifacts.GetLibrary(ctx, request.LibraryID); err != nil {
		return nil, err
	}
...
func (s *Service) Download(ctx context.Context, teamID, actorID, artifactID string) ([]byte, string, error) {
	...
	library, err := s.artifacts.GetLibrary(ctx, artifact.LibraryID)
	if err != nil {
		return nil, "", err
	}
	if library.TeamID != teamID {
		return nil, "", fmt.Errorf("artifact is outside actor team")
	}
	content, err := s.store.Get(ctx, artifact.StorageKey)
	if err != nil {
		return nil, "", err
	}
	if files.Digest(content) != artifact.Digest {
