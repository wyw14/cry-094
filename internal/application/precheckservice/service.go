...
func (s *Service) Review(ctx context.Context, planID, reviewerID, reason string, approve bool) error {
	...
	owner, err := s.teams.Get(ctx, plan.TeamID)
	if err != nil {
		return err
	}
	if !owner.Can(reviewerID, team.PermissionReviewPlan) {
		return fmt.Errorf("review permission required")
	}
	if approve {
...
func (s *Service) Sign(ctx context.Context, planID, reviewerID string) error {
	...
	if plan.ReviewedBy != reviewerID {
		return fmt.Errorf("approved reviewer must request signing")
	}
